...
def main():
    ...
    ###RUNING AUTO ENCODER:
    # Run Auto-Encoder anomaly ditection
    ...
    try:
        ...
        print(day_clean.columns)
        print(day_clean.head(10))
        day_numeric_copy=day_numeric.copy()
        encoded_d = pd.get_dummies(day_numeric_copy, 
                                   columns=['yr', 'season', 'mnth','weekday', 'weathersit'],
                                   drop_first=True)  
        logger.info(f"Encoded shape: {encoded_d.shape}")
        
        # Scale
        logger.info("Scaling features")
        encoded_scaled = scaler.fit_transform(encoded_d)
        ...
        autoencoder.fit(encoded_scaled)
        logger.info("AutoEncoder training complete")
        
        # Predict anomalies
        anomaly_labels = autoencoder.predict(encoded_scaled)
        anomaly_scores = autoencoder.decision_scores_  # Anomaly scores (higher = more anomalous)
        
        # Map back to original indices
        encoded_df = pd.DataFrame(
            encoded_scaled,
            index=encoded_d.index
        )
        encoded_df['anomaly'] = anomaly_labels
        encoded_df['anomaly_score'] = anomaly_scores
        
        # Get anomaly indices
        anomaly_indices = encoded_df[encoded_df['anomaly'] == 1].index.tolist()
        
        logger.info(f"AutoEncoder detected {len(anomaly_indices)} anomalies "
                   f"({len(anomaly_indices)/len(day_clean)*100:.1f}% of data)")
        
        # Analyze
        auto_encoder_result = analyze_anomalies(day_clean, anomaly_indices, logger)
        
        # Compare with clustering anomalies
        clustering_anomalies = set(anomalies)
        autoencoder_anomalies = set(anomaly_indices)
        
        overlap = clustering_anomalies & autoencoder_anomalies
        clustering_only = clustering_anomalies - autoencoder_anomalies
        autoencoder_only = autoencoder_anomalies - clustering_anomalies
        
        print(f"\n{'='*60}")
        print("ANOMALY DETECTION COMPARISON")
        print(f"{'='*60}")
        print(f"Clustering detected: {len(clustering_anomalies)}")
        print(f"AutoEncoder detected: {len(autoencoder_anomalies)}")
        print(f"Overlap (both methods): {len(overlap)}")
        print(f"Clustering only: {len(clustering_only)}")
        print(f"AutoEncoder only: {len(autoencoder_only)}")
